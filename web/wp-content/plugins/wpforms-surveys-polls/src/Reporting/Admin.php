...
class Admin {
	...
	public function init() {
		...
		$this->view  = ! empty( $_GET['view'] ) && in_array( $_GET['view'], array( 'survey', 'list' ), true ) ? $_GET['view'] : false; // WPCS: CSRF ok.
		$this->print = ! empty( $_GET['print'] ); // WPCS: XSS ok. // WPCS: CSRF ok.

		if ( empty( $_GET['page'] ) || 'wpforms-entries' !== $_GET['page'] || ! $this->view ) { // WPCS: CSRF ok.
			return;
		}
	...
	public function setup() {
		...
		// Get current form ID.
		$this->form_id = ! empty( $_GET['form_id'] ) ? absint( $_GET['form_id'] ) : false; // WPCS: CSRF ok.
		...
		$this->urls = array(
			...
			'form-preview'        => wpforms()->preview->form_preview_url( $this->form_id ),
		...
		if ( ! empty( $this->field_ids ) && ( 'list' === $this->view || 'survey' === $this->view && ! empty( $_GET['field_id'] ) ) ) { // WPCS: CSRF ok.

			$specific_id = false;

			if ( ! empty( $_GET['field_id'] ) ) { // WPCS: CSRF ok.
		...
		if ( isset( $_GET['wpforms_surveys_polls_delete_cache'] ) && current_user_can( apply_filters( 'wpforms_manage_cap', 'manage_options' ) ) ) { // WPCS: CSRF ok.
	...
	public function entry_list_preview( $form_data, $entries_list ) {
		...
		<div id="wpforms-survey-preview">
			<div id="wpforms-survey-report">
				<?php echo $this->display_loader( true ); // WPCS: XSS ok. ?>
			</div>
			<div class="btn-wrap">
