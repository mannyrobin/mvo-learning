...
<div class="pp-hotspot">
	<div class="pp-hotspot-container">
		...
		<?php foreach( $settings->markers_content as $i => $marker) { 
			$marker_title = ! empty( $marker->marker_title ) ? $marker->marker_title : __('Marker ' . ( $i + 1 ), 'bb-powerpack');
			?>
			<?php if( !empty($marker_title) ) {?>
				<?php if( 'yes' == $settings->enable_tour ) { $enable_tour = " data-pptour=" . ($i+1); }else{ $enable_tour = ""; }?>
