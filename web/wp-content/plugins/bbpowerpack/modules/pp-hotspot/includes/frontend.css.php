...
.fl-node-<?php echo $id; ?> span.pp-marker-title {
	<?php ( 'no' == $settings->admin_title_preview) ? 'display: none' : 'display: block' ; ?>
}
...
<?php foreach( $settings->markers_content as $i => $marker) {?>
	.fl-node-<?php echo $id; ?> .pp-hotspot-marker.pp-marker-<?php echo ($i+1);?> {
		...
		<?php if ( !empty($marker->marker_bg_color) ){?>background: <?php echo pp_get_color_value( $marker->marker_bg_color ); ?>;<?php } ?>
		<?php if ( !empty($marker->marker_border_single_color) ){?>border-color: <?php echo pp_get_color_value( $marker->marker_border_single_color ); ?>;<?php } ?>
	}
