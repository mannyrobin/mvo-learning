...
class SchemaServer {
        ...
        const API_SERVER = "https://api.schemaapp.com/";
        ...
        public function getResource($uri = '', $pretty = false) {
				...
				$this->page_url = $resource = urldecode( $resource );

				$this->readLink($resource);

                $this->transient_id = $transient_id = 'HunchSchema-Markup-' . md5($resource);
        ...
        protected function readLink($uri = '') {
                ...
                $graph = str_replace( array( 'http://schemaapp.com/db/', 'https://schemaapp.com/db/' ), '', $this->options['graph_uri'] );

				$this->api_url = $this::DATA_SERVER . $graph . '/' . trim(base64_encode($uri), '=');
