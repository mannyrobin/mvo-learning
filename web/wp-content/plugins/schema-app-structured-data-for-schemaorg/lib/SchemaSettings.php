...
//            'http://schema.org/Person <br/>Comment author', // Title 
    ...
    public function section_debug() {
		...
		if ( count( $_POST ) ) {
			if ( count( $_POST ) && ! empty( $_POST['debug_delete_transient'] ) ) {
				if ( $_POST['debug_delete_transient'] == 'all' ) {
					...
					delete_transient( $_POST['debug_delete_transient'] );

					printf( '<div class="notice notice-success is-dismissible"> <p>Debugging: Schema cache deleted with Id: %s</p> </div>', $_POST['debug_delete_transient'] );
				}
			}
		...
		print '<h3>Cached Schema Items</h3>';
		...
		if ( $transients ) {
			...
			foreach ( $transients as $transient ) {
				...
				printf( "<li> <p>Id: %s</p> <p>Expiry: %s</p> <p>Data: <br> <textarea rows='5' class='large-text code'>%s</textarea></p> <p>Action: <button type='submit' class='button button-secondary' name='debug_delete_transient' value='%s'>Delete</button></p> </li>", $transient_id, $transient_timeout, print_r( get_transient( $transient_id ), true ), $transient_id );
			}
		...
		if ( file_exists( $rest_api_cache_log_file ) ) {
			printf( '<textarea rows="20" class="large-text code">%s</textarea>', file_get_contents( $rest_api_cache_log_file ) );
