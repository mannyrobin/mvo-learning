    ...
    public function render_meta_box_content($post) {
                ...
                <?php 
					if ( $markup_final )
					{
						if ( empty($replacelink) ) {
							...
							<?php endif; ?>
							<input type="hidden" id="resourceURI" value="<?php echo get_permalink($post->ID); ?>#<?php print $schemaObj->schemaType; ?>"/>
        ...
        <p>
			...
			<div class="MetaSchemaMarkup" style="position: relative;">
				...
				<textarea class="large-text metadesc" rows="6" id="schema_new_field" name="schema_new_field" data-id="<?php print $post->ID; ?>" <?php print $MarkupCustom ? '' : 'disabled'; ?>><?php print $markup_final ? esc_attr($jsonLd) : ''; ?></textarea>
				<?php if ( $markup_final ): ?>
			...
			<table class="widefat fixed wp-list-table">
				<tbody>
					...
					<tr>
						...
						<td>
							<?php print $schema_server->page_url; ?>
						</td>
					</tr>
					<tr>
						<td style="width: 30%;">
					...
					<tr>
						...
						<td>
							<?php print $schema_server->transient_id; ?>
						</td>
					</tr>
					<tr>
						<td style="width: 30%;">
						...
						<td>
							<a href="<?php print $schema_server->api_url; ?>" target="_blank"><?php print $schema_server->api_url; ?></a>
						</td>
					</tr>
    ...
    public function ActionSavePost($PostId) {
        ...
        if (isset($_POST['schema_inner_custom_box_nonce'])) {
            ...
            if (isset($_POST['HunchSchemaEnableMarkup'])) {
                update_post_meta($PostId, '_HunchSchemaEnableMarkup', $_POST['HunchSchemaEnableMarkup']);
            } else {
