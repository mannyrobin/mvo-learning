...
class Settings
{
	...
	public static function getSettings()
	{
		...
		/*if($saved['enable_search'] == true){
            $bg_color = new PHPColors\Color($saved['bg_color']);
            if(!in_array('search_bg',$saved) || empty($saved['search_bg'])){
                $saved['css']['search_bg'] = '#'.$bg_color->darken(20);
            }
            if(!in_array('search_btn_color',$saved) || empty($saved['search_btn_color'])){
                $saved['css']['search_btn_color'] = $saved['search_btn_color'];
            }
        }*/
		return $saved;
	}
