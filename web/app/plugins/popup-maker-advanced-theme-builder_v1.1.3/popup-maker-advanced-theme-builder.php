...
class PUM_ATB {
	...
	public static function init() {
		...
		require_once static::$DIR . 'includes/pum-atb-functions.php';
		...
		if ( class_exists( 'PopMake_License' ) ) {
			new PopMake_License( __FILE__, 'Advanced Theme Builder', static::$VER, 'WP Popup Maker' );
		}
	}
	...
	public static function theme_css( $styles, $popup_theme_id, $theme ) {
		...
		if ( ! empty( $extra_styles ) ) {
			static::$extra_styles .= "/* Popup Theme " . $popup_theme_id . " Additional Styles */\r\n";
			static::$extra_styles .= $extra_styles;
		}
...
html.pum-open-fixed .pum-overlay::after { margin-right: 0; }";
	...
	public static function scripts() {
		...
		if ( is_admin() && popmake_is_admin_popup_theme_page() ) {
			wp_enqueue_media();
			wp_enqueue_script( 'pum-atb-admin', static::$URL . 'assets/js/admin' . $suffix . '.js', array( 'popup-maker-admin' ), static::$VER, true );
	...
	public static function background_fields( $post_id ) {
		...
		<tr>
			<th scope="row">
				<label for="<?php esc_attr_e( $field_name ); ?>_bg_type">
			...
			<td><?php
				...
				?>
				<select id="<?php esc_attr_e( $field_name ); ?>_bg_type" name="<?php esc_attr_e( $field_name ); ?>_bg_type" data-toggle='<?php echo wp_json_encode( $select_option_sections ); ?>'>
					<?php foreach ( $bg_type_options as $value => $label ) : ?>
						<option value="<?php esc_attr_e( $value ); ?>" <?php selected( $value, $values['bg_type'] ); ?>><?php esc_html_e( $label ); ?></option>
					<?php endforeach; ?>
				</select>
			</td>
		</tr>
		...
		<div class="<?php echo $which; ?>-bg_image">
			<table class="form-table">
				...
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_image">
						...
						<div class="pum-image-field <?php echo $values['bg_image'] > 0 ? '' : 'pum-image-empty'; ?>">
							...
							<div class="pum-image-preview">
								<div class="pum-image-preview-img"><?php
									...
									<img src="<?php esc_attr_e( $img_src ); ?>" height="100" />
								</div>
								<select name="<?php esc_attr_e( $field_name ); ?>_bg_image_src">
									<?php foreach ( PUM_ATB::get_image_sizes() as $size => $atts ) :
										$url = wp_get_attachment_image_src( $values['bg_image'], $size );
										$label = ucwords( str_replace( array(
												'_',
												'-',
											), ' ', $size ) ) . ' (' . implode( 'x', $atts ) . ')'; ?>
										<option value="<?php esc_attr_e( $url[0] ); ?>" <?php selected( $values['bg_image_src'], $url[0] ); ?>><?php esc_html_e( $label ); ?></option>
									<?php endforeach; ?>
								</select>
								<br />
							...
							<input type="hidden" name="<?php esc_attr_e( $field_name ); ?>_bg_image" id="<?php esc_attr_e( $field_name ); ?>_bg_image" value="<?php esc_attr_e( $values['bg_image'] ) ?>" />
						</div>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_repeat"><?php _e( 'Repeat', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td><?php
						...
						<select name="<?php esc_attr_e( $field_name ); ?>_bg_repeat" id="<?php esc_attr_e( $field_name ); ?>_bg_repeat">
							<?php foreach ( $bg_repeat_options as $value => $label ) : ?>
								<option value="<?php esc_attr_e( $value ); ?>" <?php selected( $value, $values['bg_repeat'] ); ?>><?php esc_html_e( $label ); ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_position"><?php _e( 'Position', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td><?php
						...
						<select name="<?php esc_attr_e( $field_name ); ?>_bg_position" id="<?php esc_attr_e( $field_name ); ?>_bg_position">
							<?php foreach ( $bg_position_options as $value => $label ) : ?>
								<option value="<?php esc_attr_e( $value ); ?>" <?php selected( $value, $values['bg_position'] ); ?>><?php esc_html_e( $label ); ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_attachment"><?php _e( 'Attachment', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td><?php
						...
						<select name="<?php esc_attr_e( $field_name ); ?>_bg_attachment" id="<?php esc_attr_e( $field_name ); ?>_bg_attachment">
							<?php foreach ( $bg_attachment_options as $value => $label ) : ?>
								<option value="<?php esc_attr_e( $value ); ?>" <?php selected( $value, $values['bg_attachment'] ); ?>><?php esc_html_e( $label ); ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_size"><?php _e( 'Size', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td><?php
						...
						<select name="<?php esc_attr_e( $field_name ); ?>_bg_size" id="<?php esc_attr_e( $field_name ); ?>_bg_size">
							<?php foreach ( $bg_size_options as $value => $label ) : ?>
								<option value="<?php esc_attr_e( $value ); ?>" <?php selected( $value, $values['bg_size'] ); ?>><?php esc_html_e( $label ); ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
		...
		<div class="<?php echo $which; ?>-bg_color">
			<table class="form-table">
				...
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_color"><?php _e( 'Color', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td>
						<input type="text" name="<?php esc_attr_e( $field_name ); ?>_bg_color" id="<?php esc_attr_e( $field_name ); ?>_bg_color" value="<?php esc_attr_e( $values['bg_color'] ) ?>" class="pum-color-picker color-picker background-color bg-color" />
					</td>
				</tr>
				<tr class="bg_opacity">
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_opacity"><?php _e( 'Opacity', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td>
						<input type="text" readonly value="<?php esc_attr_e( $values['bg_opacity'] ) ?>" name="<?php esc_attr_e( $field_name ); ?>_bg_opacity" id="<?php esc_attr_e( $field_name ); ?>_bg_opacity" class="popmake-range-manual" step="1" min="0" max="100" data-force-minmax=true />
		...
		<div class="<?php echo $which; ?>-bg_overlay">
			<table class="form-table">
				...
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_overlay_color"><?php _e( 'Color', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td>
						<input type="text" name="<?php esc_attr_e( $field_name ); ?>_bg_overlay_color" id="<?php esc_attr_e( $field_name ); ?>_bg_overlay_color" value="<?php esc_attr_e( $values['bg_overlay_color'] ) ?>" class="pum-color-picker color-picker background-color bg-color" />
					</td>
				</tr>
				<tr class="bg_overlay_opacity">
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_overlay_opacity"><?php _e( 'Opacity', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td>
						<input type="text" readonly value="<?php esc_attr_e( $values['bg_overlay_opacity'] ) ?>" name="<?php esc_attr_e( $field_name ); ?>_bg_overlay_opacity" id="<?php esc_attr_e( $field_name ); ?>_bg_overlay_opacity" class="popmake-range-manual" step="1" min="0" max="100" data-force-minmax=true />
						<span class="range-value-unit regular-text"><?php _e( '%' ); ?></span>
					</td>
				</tr>
				</tbody>
		...
		<div class="<?php echo $which; ?>-bg_video">
			<table class="form-table">
				...
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_video"><?php _e( 'Video (MP4)', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td>
						<div class="pum-video-field pum-video-empty">
							...
							<input type="hidden" name="<?php esc_attr_e( $field_name ); ?>_bg_video" id="<?php esc_attr_e( $field_name ); ?>_bg_video" value="<?php esc_attr_e( $values['bg_video'] ) ?>" />
						</div>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_video_webm"><?php _e( 'Video (WebM)', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td>
						<div class="pum-video-field pum-video-empty">
							<a class="pum-video-select" href="javascript:void(0);" onclick="return false;"><?php _e( 'Select Video', 'popup-maker-advanced-theme-builder' ); ?></a>
							...
							<input type="hidden" name="<?php esc_attr_e( $field_name ); ?>_bg_video_webm" id="<?php esc_attr_e( $field_name ); ?>_bg_video_webm" value="<?php esc_attr_e( $values['bg_video_webm'] ) ?>" />
						</div>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_video_fallback">
							<?php _e( 'Image', 'popup-maker-advanced-theme-builder' ); ?>
						</label>
					</th>
					<td>
						<input type="text" name="<?php esc_attr_e( $field_name ); ?>_bg_video_fallback" id="<?php esc_attr_e( $field_name ); ?>_bg_video_fallback" value="<?php esc_attr_e( $values['bg_video_fallback'] ) ?>" />
						<input id="<?php esc_attr_e( $field_name ); ?>_bg_video_fallback_button" type="button" class="button" value="<?php _e( 'Select Image', 'popup-maker-advanced-theme-builder' ); ?>" />
        ...
        */ ?><?php #endregion ?>
		...
		<div class="<?php echo $which; ?>-bg_parallax">
			<table class="form-table">
				...
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_parallax_image">
							<?php _e( 'Image', 'popup-maker-advanced-theme-builder' ); ?>
						</label>
					</th>
					<td>
						<div class="pum-image-field pum-image-empty">
							...
							<div class="pum-image-preview">
								...
								<select name="<?php esc_attr_e( $field_name ); ?>_bg_parallax_image_src">
									<?php foreach( PUM_ATB::get_image_sizes() as $size => $atts ) :
										$label = ucwords( str_replace( array( '_', '-' ), ' ', $size ) ) . ' (' . implode( 'x', $atts ) . ')'; ?>
										<option value="<?php esc_attr_e( $size ); ?>" <?php selected( $values['bg_parallax_image_src'], $size ); ?>><?php esc_html_e( $label ); ?></option>
									<?php endforeach; ?>
								</select>
								<br />
								<a class="pum-image-edit" href="javascript:void(0);" onclick="return false;"><?php _e( 'Edit', 'popup-maker-advanced-theme-builder' ); ?></a>
							...
							<input type="hidden" name="<?php esc_attr_e( $field_name ); ?>_bg_parallax_image" id="<?php esc_attr_e( $field_name ); ?>_bg_parallax_image" value="<?php esc_attr_e( $values['bg_parallax_image'] ) ?>" />
						</div>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="<?php esc_attr_e( $field_name ); ?>_bg_parallax_speed"><?php _e( 'Speed', 'popup-maker-advanced-theme-builder' ); ?></label>
					</th>
					<td><?php
						...
						<select name="<?php esc_attr_e( $field_name ); ?>_bg_parallax_speed" id="<?php esc_attr_e( $field_name ); ?>_bg_parallax_speed">
							<?php foreach ( $bg_parallax_speed_options as $value => $label ) : ?>
								<option value="<?php esc_attr_e( $value ); ?>" <?php selected( $value, $values['bg_parallax_speed'] ); ?>><?php esc_html_e( $label ); ?></option>
							<?php endforeach; ?>
						</select>
					</td>
				</tr>
